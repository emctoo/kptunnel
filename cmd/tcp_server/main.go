...
func ParseOpt(
 ...
 -1: infinity
  ...
  N: packet count`)
	...
	param := kptunnel.TunnelParam{
		Pass: pass, Mode: mode, MaskedIP: maskIP, EncPass: encPass, EncCount: *encCount, KeepAliveInterval: *interval * 1000,
...
func ParseOptServer(mode string, args []string) {
	...
	switch mode {
	case "server":
		kptunnel.StartServer(param, forwardList)
	case "r-server":
		kptunnel.StartReverseServer(param, forwardList)
	}
}
