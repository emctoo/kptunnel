...
func ParseOptServer(mode string, args []string) {
	...
	log.Printf("server forwards: (nil? %t, len: %d) %#v", forwardList == nil, len(forwardList), forwardList)

	switch mode {
	case "wsserver":
		server := kptunnel.StartWebsocketServer(param, forwardList)
		log.Printf("start websocket server at %s ...", param.ServerInfo.String())
		if err := server.ListenAndServe(); err != nil {
			log.Fatal().Err(err)
		}
	case "r-wsserver":
		server := kptunnel.StartReverseWebSocketServer(param, forwardList)
		log.Printf("start reverse websocket server at %s ...", param.ServerInfo.String())
		if err := server.ListenAndServe(); err != nil {
			log.Fatal().Err(err)
		}
	}
